...
class bb extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    console.log("bb >> componentWillReceiveProps", nextProps);
    // this.setState({ });
  }
  static getDerivedStateFromProps(nextProps, prevState) {
    // props로 부터 파생된 state를 만드는 정적 메서드. 정적 메서드 이므로 this 롤 사용 할 수 없다.
    console.log("bb >> getDerivedStateFromProops", nextProps, prevState);
    return null;
  }
  shouldComponentUpdate(nextProps, nextState) {
    // 컴포넌트 UI 를 업데이트 여부를 정한다. return true이면 ui 업데이트 하고 return false 이면 ui 업데이트 안함
    console.log("bb >> shouldComponentUpdate", nextProps, nextState);
    return true; // true or false
  }
  componentWillUpdate(nextProps, nextState) {
    console.log("bb >> componentWillUpdate", nextProps, nextState);
  }
  componentDidUpdate(prevProps, prevState) {
    // 업데이트 완료 후에 실행됨 : 여러번, state 가 변경될 때마다
    console.log("bb >> componentDidUpdate", prevProps, prevState);
  }
